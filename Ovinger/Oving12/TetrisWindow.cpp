...
void TetrisWindow::drawGridMatrix() {
    for(int row = 0; row < tetrisGridHeight; row++) {
        draw_line({0,row*blockSize}, {tetrisGridWidth*blockSize, row*blockSize});
        for(int column = 0; column < tetrisGridWidth; column++) {
            draw_line({column*blockSize,0}, {column*blockSize, tetrisGridHeight*blockSize});
...
void TetrisWindow::drawNextTetromino() {
    Point topLeftCorner = {(tetrisGridWidth+2), 10};
    int size = nextTetromino.getMatrixSize();

    for(int row = 0; row < size; row++) {
        for(int column = 0; column < size; column++) {
            TetrominoType field = nextTetromino.getBlock(row, column);
            if(field != TetrominoType::NONE) {
                draw_rectangle(Point{(topLeftCorner.x + column) * blockSize, (topLeftCorner.y + row) * blockSize}, blockSize, blockSize, tetrominoColorMap.at(field), Color::black);
            }
        }
...
void TetrisWindow::hardDrop() {
    auto position = currentTetromino.getPosition();
    int matrixSize = currentTetromino.getMatrixSize();
    bool canMoveDown = true;

    while (!tetrominoCrashesAt({position.x,position.y+1})) {
        score++;
        moveTetrominoDown();
        string scoreText{"Score: "};
        scoreText += std::to_string(score);
        scoreBox.setText(scoreText.c_str());
        position = currentTetromino.getPosition(); // oppdater posisjon etter bevegelse
    }
}
