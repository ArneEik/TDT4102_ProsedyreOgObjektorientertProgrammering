...
void CourseCatalog::saveToFile(const filesystem::path& filename) const
{
	ofstream out{filename};

	if (!out)
	{
		error("Couldn't open file: ", filename.string());
	}
...
void CourseCatalog::loadFromFile(const filesystem::path& filename)
{
	...
	while (getline(in, line)) {
		...
		// Henter ut de to delene av linjen, som er separert av komma ','
		getline(ss, emneKode, ',');
		getline(ss, emneNavn, ',');
