...
string Forecast::capitalize(string word)
{
  ...
  // already there and replace it with your own.
  if(word.size() <= 0){
    throw std::out_of_range("ther is no word form Forecast::capitalize");
  }
...
void Forecast::temp_statistics()
{
  ...
  double totalTemp;
  for(auto& d:days){
    max_temp = d->temp >max_temp ? d->temp : max_temp;
    min_temp = d->temp <min_temp ? d->temp : min_temp;
    totalTemp += d->temp;
  }
  avg_temp = totalTemp/days.size();
...
void Forecast::wind_statistics()
{
  ...
  double max_wind = 0;
  for (auto& d : days) {
      if (d->wind_speed > max_wind) {
          max_wind = d->wind_speed;
          windiest_day = d->day;
      }
  }
