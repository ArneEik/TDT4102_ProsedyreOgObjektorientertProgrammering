...
// already there and replace it with your own.
    begin = {p1.x<p2.x ? p1.x:p2.x, p1.y<p2.y ? p1.y : p2.y};
    end = {p1.x>p2.x ? p1.x:p2.x, p1.y>p2.y ? p1.y : p2.y};
// END: T4
}
...
void CameraController::handleInput(Context &ctx) {
    ...
    int scrollSpeed =
        window.is_key_down(KeyboardKey::LEFT_SHIFT) ?
            m_scrollSpeed * 5 : m_scrollSpeed*3;
...
// already there and replace it with your own.
    std::vector<int> state{
                    window.is_key_down(KeyboardKey::W) ||  window.is_key_down(KeyboardKey::UP) ? 1 : 0,
                    window.is_key_down(KeyboardKey::A) ||  window.is_key_down(KeyboardKey::LEFT) ? 1 : 0,
                    window.is_key_down(KeyboardKey::S) ||  window.is_key_down(KeyboardKey::DOWN) ? 1 : 0,
                    window.is_key_down(KeyboardKey::D) ||  window.is_key_down(KeyboardKey::RIGHT) ? 1 : 0};
    camera.translateY((state[2]-state[0])*scrollSpeed);
    camera.translateX((state[3]-state[1])*scrollSpeed);
// END: T5
}
...
// already there and replace it with your own.
    ...
    size_t tileIndex = Y*width + X;
    if (tileIndex<this->walkable.size()){
        this->walkable.at(tileIndex) = walkable;
    }
// END: T7
}
...
// already there and replace it with your own.
    ...
    std::string path;
    bool walkebale;
    is>>id>>path>>walkebale;
    return TileDescriptor{id, path, walkebale};
// END: T11
}
...
// already there and replace it with your own.
    std::ifstream is(descriptor_file_path);
    if(!is.is_open()){
        throw std::runtime_error("fann ikkje: " + descriptor_file_path.string());
    }
    std::string line;
    while (std::getline(is,line))
    {   
        auto content = process_line(line);
        imgpool.add_tile(content.id, Tile(content.id, content.walkable, "tiles/" + content.filename));
    }
    is.close();
    return imgpool;
// END: T12
