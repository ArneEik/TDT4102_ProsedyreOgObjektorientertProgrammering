...
{
// BEGIN: T8
    double o = atan2(position.y,position.x); //Thetha = o

    FloatingPoint p1 = {
    position.x + size * std::cos(o),
    position.y + size * std::sin(o)
    };
    FloatingPoint p2 = {
    position.x + size/2.0 * std::cos(o + 2.0/3.0 * M_PI),
    position.y + size/2.0 * std::sin(o + 2.0/3.0 * M_PI)
    };
    FloatingPoint p3 = {
    position.x + size/2.0 * std::cos(o - 2.0/3.0 * M_PI),
    position.y + size/2.0 * std::sin(o - 2.0/3.0 * M_PI)
...
double getDistance(FloatingPoint a, FloatingPoint b){
...
void Dove::makeFriendsAndFoes(vector<shared_ptr<Bird>> &birds)
{
    ...
    for(auto& other: birds) {
        bool isFriend(  other.get() != this &&
                        isInstanceOf<Dove>(other.get()) &&
                        getDistance(other->getPosition(),this->getPosition()) <= FRIEND_RADIUS
                        );
        bool isFoe(  other.get() != this &&
                        isInstanceOf<Hawk>(other.get()) &&
                        getDistance(other->getPosition(),this->getPosition()) <= AVOID_RADIUS
                        );
        if (isFoe) {
...
{
// BEGIN: T7
FloatingPoint cohesion = calculateCohesion();
FloatingPoint separation = calculateSeparation();
FloatingPoint alignment = calculateAlignment();
FloatingPoint avoidance = calculateAvoidance();
FloatingPoint newVelocity = velocity + cohesion + separation
+ alignment + avoidance;
...
std::map<int , TDT4102::Color> colorMap{
    {0, TDT4102::Color::blue},
    {1, TDT4102::Color::blue_violet},
    {2, TDT4102::Color::yellow},
    {3, TDT4102::Color::red},
    {4, TDT4102::Color::hot_pink},
    {5, TDT4102::Color::green_yellow},
    {6, TDT4102::Color::burly_wood},
};
TDT4102::Color randColor(){
    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<> dis(0,colorMap.size()-1);
    try
    {
        return colorMap.at(dis(gen));
    }
    catch(const std::exception& e)
    {
        std::cerr << e.what() << '\n';
        return TDT4102::Color::hot_pink;
    }
}
...
{
    auto theDoves = getDoves(birds);
    for(auto& dove : theDoves) {
        TDT4102::Color randomColor = randColor();
        vector<shared_ptr<Dove>> flock;
        getFlock(dove, flock);
        
        if (flock.empty()) continue; 

        TDT4102::Color firstColor = flock.front()->getDisplayColor();
        bool allSameColor = true;

        for (auto& flockmember : flock) {
            if (flockmember->getDisplayColor() != firstColor) {
                allSameColor = false;
                break;
            }
        }
        ...
        if (!allSameColor) {
            for (auto& flockmember : flock) {
                flockmember->setColor(randomColor);
            }
        }
    }
}
...
// BEGIN: T10
    ...
    bool DisDown = window.is_key_down(KeyboardKey::D);
    bool CurrentMouseState = window.is_key_down(KeyboardKey::D);//window.is_left_mouse_button_down();
