...
void RobotGrid::draw_grid_lines()
{
    ...
    for (int y = 0; y <= rows; ++y) {
        Point rowPointStart = {x_pos, y_pos + y * cell_height};
        Point rowPointEnd = {x_pos + cols * cell_width, y_pos + y * cell_height};
        window.draw_line(rowPointStart, rowPointEnd);
    }
    for (int x = 0; x <= cols; ++x) {
        Point colPointStart = {x_pos + x * cell_width, y_pos};
        Point colPointEnd = {x_pos + x * cell_width, y_pos + rows * cell_height};
        window.draw_line(colPointStart, colPointEnd);
    }
    // END: G1
}
...
Point RobotGrid::get_grid_cell_center_coord(int x, int y) const
{
  ...
  int centerY = y_pos + y *cell_width + cell_width/2;
  int centerX = x_pos + x *cell_height + cell_height/2;
...
void RobotGrid::make_robot(string name, Point pos, Color color)
{
  ...
  // already there and replace it with your own.
  auto newRobot = make_unique<Robot>(name, pos, color);
  robots.insert({name, std::move(newRobot)});
...
void RobotGrid::draw_robots()
{
  ...
  for(auto& robot:robots){
    int x = robot.second->pos.x;
    int y = robot.second->pos.y;
    TDT4102::Point center = get_grid_cell_center_coord(x,y);
    TDT4102::Color color = robot.second->color;
    std::string name = robot.second->name;
    window.draw_circle(center,std::min(cell_width/2,cell_height/2),color,color);
    
    TDT4102::Point text_pos = { center.x - cell_width / 2, center.y };
    window.draw_text(text_pos, name, TDT4102::Color::black);
    }
...
void RobotGrid::delete_robot(string name)
{
  ...
  if(robots.count(name) == 0){
    throw std::runtime_error("not a robot");
  }
  robots.erase(name);
...
void RobotGrid::rename_robot(string name, string new_name)
{
  ...
  robots.insert({new_name, std::move(robots.at(name))});
  delete_robot(name);
  // END: G9
}
...
void RobotGrid::check_name_available(string name) const
{
  ...
  // already there and replace it with your own.
  if(robots.count(name) != 0){
    throw std::out_of_range("the name: " + name + ", is taken!");
  }
...
void RobotGrid::check_name_exists(string name) const
{
  ...
  // already there and replace it with your own.
    ...
    if(robots.count(name) == 0){
    throw std::out_of_range("the robot: " + name + ", does not exist!");
  }
...
void RobotGrid::check_coord_empty(Point p, string name, bool is_moving) const
{
  ...
  // already there and replace it with your own.
  for(auto& robot : robots){
    Point takenPos = robot.second->pos;
    std::string robotName = robot.second->name;
    if(takenPos.x == p.x && takenPos.y == p.y){
      if(is_moving && robotName == name){
        continue;
      }else{
        throw invalid_argument("position " + std::to_string(p.x) + "," +  std::to_string(p.y) + "is ocupied by " + robot.second->name);
      }
  }  
  }
  // END: G13
